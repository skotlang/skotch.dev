...
| Try/catch | [§7.4.5](https://kotlinlang.org/spec/expressions.html#try-expression) | Done | Full exception tables, try-as-expression, multi-statement bodies |
| Throw expression | [§7.4.6](https://kotlinlang.org/spec/expressions.html#throw-expressions) | Done | `athrow` emission with `Ty::Nothing` return type |
