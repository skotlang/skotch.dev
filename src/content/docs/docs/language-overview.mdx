...
295+ test fixtures are validated across JVM, DEX, LLVM IR, klib, and
native targets.
</Aside>
...
| [Class declarations](https://kotlinlang.org/spec/declarations.html#class-declaration) | §4.5 | Primary constructor, field access, instance methods, `init` blocks, mutable `var` field writeback, `override fun toString()` |
| [Increment/decrement](https://kotlinlang.org/spec/expressions.html#postfix-increment-expression) | §7.3.4 | `x++`, `x--` postfix operators on locals and class fields |
| [Object declarations](https://kotlinlang.org/spec/declarations.html#object-declaration) | §4.5.4 | `object Singleton { fun greet() {} }` — singleton methods via `Singleton.method()` |
| [Enum classes](https://kotlinlang.org/spec/declarations.html#enum-class-declaration) | §4.5.7 | `enum class Color { RED, GREEN, BLUE }` — entries via `Color.RED`, works in `when` |
...
Class `var` fields can be mutated in methods, and changes persist:
